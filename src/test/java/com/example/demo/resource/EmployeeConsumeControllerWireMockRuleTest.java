...
public class EmployeeConsumeControllerWireMockRuleTest {

	@Rule
	public WireMockRule wireMockRule = new WireMockRule(8085);//wireMockConfig().port(8085));
...
//		wm.start();
		...
		wireMockRule.stubFor(get(urlEqualTo("/hello")).willReturn(
				aResponse()
				.withStatus(200)
	            .withHeader("Content-Type", "text/plain")
	            .withBody("Welcome to Hello World!")));
		
		wireMockRule.stubFor(get(urlPathMatching("/products"))
				.willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")
						.withBody("{ \"id\": \"1\",\"name\": \"Honey\"},{\"id\": \"2\",\"name\": \"Almond\" }")));
		wireMockRule.start();
	}
